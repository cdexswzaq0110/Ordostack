...
- ClearML（docs/clearml-mlops.md 已規劃）此時才引入：experiment tracking + model registry 取代本地 JSON registry。**介面已對齊**：`model_registry.py` 的 `active_model_path()` 抽象讓後端換 registry 不動服務程式。
